 ...
 * @param tokenTypes
 * @param grammarRepository 注册仓库,也就是刚开始默认生成的
...
export class PartialScopeDependency {
    constructor(public readonly scopeName: string, public readonly include: string) {}
...
/**
 * Collect a specific dependency from the grammar's repository 收集仓库中的包含以来?
 */
export function collectSpecificDependencies(
    result: ScopeDependencyCollector,
    baseGrammar: IRawGrammar,
    selfGrammar: IRawGrammar,
    include: string,
...
class ScopeMetadataProvider {
    ...
    constructor(
        /**0*/ initialLanguage: number,
        /**注册,缩减接口了 */ themeProvider: IThemeProvider,
        /**null */ embeddedLanguages: IEmbeddedLanguagesMap | null
    ) {
...
export class Grammar implements IGrammar, IRuleFactoryHelper, IOnigLib {
    ...
    constructor(
        /**原始语法*/ grammar: IRawGrammar,
        /**0 */ initialLanguage: number,
        /**null */ embeddedLanguages: IEmbeddedLanguagesMap | null,
        /**null */ tokenTypes: ITokenTypeMap | null,
        grammarRepository: IGrammarRepository & IThemeProvider,
        onigLib: IOnigLib
    ) {
